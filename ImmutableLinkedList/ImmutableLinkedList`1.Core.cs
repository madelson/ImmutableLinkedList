...
/// </summary>
[CollectionBuilder(typeof(ImmutableLinkedList), nameof(ImmutableLinkedList.CreateRange))]
public readonly partial struct ImmutableLinkedList<T> : IReadOnlyCollection<T>, IEquatable<ImmutableLinkedList<T>>
{
    ...
    internal static ImmutableLinkedList<T> Create(T value) => new(new(value), 1);
    ...
    internal static ImmutableLinkedList<T> CreateRange(ReadOnlySpan<T> values)
    {
        Node? head = null;
        for (var i = values.Length - 1; i >= 0; --i)
        {
            head = new(values[i]) { Next = head };
        }
        return new(head, values.Length);
    }
    ...
    public void Deconstruct(out T head, out ImmutableLinkedList<T> tail)
    {
        ...
        tail = new(this._head.Next, this._count - 1);
    }
    ...
    private static void CopyNonEmptyRange(Node head, Node? last, out Node newHead, out Node newLast)
    {
        ...
        var newCurrent = newHead = new(head.Value);
        for (var current = head.Next; current != last; current = current.Next)
        {
            newCurrent = newCurrent.Next = new(current!.Value);
        }
        newLast = newCurrent;
    }
}
